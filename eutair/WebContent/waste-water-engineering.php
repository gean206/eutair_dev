<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<title>AMC - Eutair Equipments</title>
<meta
	content="width=device-width, initial-scale=1.0"
	name="viewport">
<meta
	content="EutAir Equipments, Elgi,Ingersoll Rand ,Gardner Denver ,Atlas Copco,Evolution,FS Curtis,Roteck,Venus ,Kaeser,Chicago pneumatics,Comptech,Mark air compressor,Delhi,Gurgaon ,Meerut,Faridabad,Ghaziabad,Sonepat,Delhi,Noida ,Greater Noida ,Manesar ,Bhiwadi,Alwar ,Moradabad ,Okhla ,Bawana ,Bahadurgarh,Ballabgarh,India ,Nepal ,Myanmar,Sri Lanka ,Bangladesh,Pakistan,Nigeria ,UAE,Dubai ,UAE ,United Arab Emirates, AMC,ANNUAL MAINTENANCE CONTRACT,SERVICE ,MAINTENANCE,REPAIR,PREVENTIVE MAINTENANCE,SCHEDULED MAINTENANCE"
	name="keywords">
<meta
	content="Authorized service provider Mark air compressor , Atlas Copco Air Compressor, Ingersoll Rand , Elgi , Gardner Denver, Chicago Pneumatics, Kaeser, Venus, Comptech, Roteck, Jaguar, Repair, Maintenance, Service"
	name="description">
<!-- Favicons -->
<link
	href="img/eut-logo/favicon.png"
	rel="icon">
<link
	href="img/eut-logo/eutair_favicon.png"
	rel="apple-touch-icon">
<!-- Google Fonts -->
<link
	href="https://fonts.googleapis.com/css?family=Poppins:300,400,500,600,700"
	rel="stylesheet">
<!-- Bootstrap CSS File -->
<link
	href="lib/bootstrap/css/bootstrap.min.css"
	rel="stylesheet">
<!-- Libraries CSS Files -->
<link
	href="lib/font-awesome/css/font-awesome.min.css"
	rel="stylesheet">
<link
	href="lib/animate/animate.min.css"
	rel="stylesheet">
<link
	href="lib/ionicons/css/ionicons.min.css"
	rel="stylesheet">
<link
	href="lib/owlcarousel/assets/owl.carousel.min.css"
	rel="stylesheet">
<!-- Main Stylesheet File -->
<link
	href="css/main.css"
	rel="stylesheet">
<!-- Custom Stylesheet File -->
<link
	href="css/custom.css"
	rel="stylesheet">
</head>
<body>
	<!--/ Nav Star /-->
	<nav
		class="navbar navbar-default navbar-trans navbar-expand-lg fixed-top">
		<div
			class="container"
			style="margin-right: unset;">
			<button
				class="navbar-toggler collapsed"
				type="button"
				data-toggle="collapse"
				data-target="#navbarDefault"
				aria-controls="navbarDefault"
				aria-expanded="false"
				aria-label="Toggle navigation">
				<span></span> <span></span> <span></span>
			</button>
			<logo></logo>
			<div
				class="navbar-collapse collapse justify-content-center"
				id="navbarDefault">
				<ul class="navbar-nav">
					<li class="nav-item"><a
						class="nav-link"
						href="index.php">Home</a></li>
					<li class="nav-item dropdown"><a
						class="nav-link dropdown-toggle"
						href="#"
						id="navbarDropdown-about"
						role="button"
						data-toggle="dropdown"
						aria-haspopup="true"
						aria-expanded="false"> About Us </a>
						<div
							class="dropdown-menu"
							aria-labelledby="navbarDropdown-about">
							<a
								class="dropdown-item"
								href="who-we-are.html">Who We Are</a> <a
								class="dropdown-item"
								href="press-release-certificate.php">Press Release &
								Certification</a>
						</div></li>
					<li class="nav-item dropdown"><a
						class="nav-link dropdown-toggle"
						href="#"
						id="navbarDropdown-product"
						role="button"
						data-toggle="dropdown"
						aria-haspopup="true"
						aria-expanded="false"> Products </a>
						<div
							class="dropdown-menu"
							aria-labelledby="navbarDropdown-product">
							<a
								class="dropdown-item"
								href="screw-air-compressor.php">Screw Air Compressor<img
								alt="Screw Air Compressor"
								src="img/eut-prod/sm-mark-compressors_logo.jpg" />
							</a> <a
								class="dropdown-item"
								href="refrigerated-air-dryer.php">Refrigerated Air Dryers<img
								alt="Refrigerated Air Dryers"
								src="img/eut-prod/sm-mark-compressors_logo.jpg" />
							</a> <a
								class="dropdown-item"
								href="reciprocating-compressor-and-dryer.php">Reciprocating Compressor & Dryer</a>
							<a
								class="dropdown-item"
								href="spare-parts.php">Spare Parts</a> <a
								class="dropdown-item"
								href="pressure-vessel.php">Pressure Vessel</a>
						</div></li>
					<li class="nav-item dropdown"><a
						class="nav-link dropdown-toggle"
						href="#"
						id="navbarDropdown-service"
						role="button"
						data-toggle="dropdown"
						aria-haspopup="true"
						aria-expanded="false">Services</a>
						<div
							class="dropdown-menu"
							aria-labelledby="navbarDropdown-service">
							<a
								class="dropdown-item"
								href="amc-service.php">Annual Maintenance Contract</a> <a
								class="dropdown-item"
								href="engineering-consultancy-service.php">Engineering Consultancy</a> <a
								class="dropdown-item"
								href="epc-turnkey-service.php">EPC-Turnkey Solutions</a> <a
								class="dropdown-item"
								href="air-audit-service.php">Air Audits</a> <a
								class="dropdown-item"
								href="customization-service.php">Product Customization</a> <a
								class="dropdown-item"
								href="training-workshop-service.php">Seminars, Webinars & Workshops</a><a
								class="dropdown-item active"
								href="waste-water-engineering.html">Wastewater Engineering</a>
						</div></li>
					<li class="nav-item"><a
						class="nav-link"
						href="contact.php">Contact</a></li>
				</ul>
			</div>
		</div>
		<contact-info class="contact-info-custom"></contact-info>
	</nav>
	<!--/ Nav End /-->
	<!--/ Intro Single star /-->
	<section class="intro-single color-background-custom">
		<div class="container">
			<div class="row">
				<div class="col-md-12 col-lg-6">
					<div class="title-single-box">
						<h1 class="title-single">Wastewater Engineering</h1>
						<span class="color-text-a">Treatment Plants</span>
					</div>
				</div>
				<div class="col-md-12 col-lg-6">
					<nav
						aria-label="breadcrumb"
						class="breadcrumb-box d-flex justify-content-lg-end">
						<ol class="breadcrumb">
							<li class="breadcrumb-item"><a
								class="anchor-color-b"
								href="index.php">Home</a></li>
							<li class="breadcrumb-item"><a
								class="anchor-color-b"
								href="#">Services</a></li>
							<li
								class="breadcrumb-item active"
								aria-current="page">Wastewater Engineering</li>
						</ol>
					</nav>
				</div>
			</div>
		</div>
	</section>
	<!--/ Intro Single End /-->
	<!--/ service Single Star /-->
	<section class="service-single nav-arrow-b color-background-custom">
		<div class="container">
			<div class="row">
				<div class="col-sm-12">
					<div
						id="service-carousel"
						class="owl-carousel owl-arrow gallery-property">
						<div class="carousel-item-b">
							<img
								src="img/eut-services/wwe-1.jpg"
								alt="Annual Maintenance Contract">
							<div class="intro-content display-table centered-custom">
								<div class="table-cell">
									<div class="container">
										<div class="row">
											<div class="col-lg-8">
												<div class="intro-body">
													<p class="intro-title-top"></p>
													<h1 class="intro-title mb-4 tagline-size">
														<span class="color-eutair"></span> <br> <span style="color: white;">All your water solutions</span>
													</h1>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
						<div class="carousel-item-b">
							<img
								src="img/eut-services/wwe-2.jpg"
								alt="Annual Maintenance Contract">
							<div class="intro-content display-table centered-custom">
								<div class="table-cell">
									<div class="container">
										<div class="row">
											<div
												class="col-lg-9"
												style="padding-bottom: 22em;">
												<div class="intro-body tagline-size">
													<h1 class="intro-title mb-4">
														Sewage Treatment Plants
													</h1>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
						<div class="carousel-item-b">
							<img
								src="img/eut-services/wwe-3.jpg"
								alt="Annual Maintenance Contract">
							<div class="intro-content display-table centered-custom">
								<div class="table-cell">
									<div class="container">
										<div class="row">
											<div
												class="col-lg-9"
												style="padding-bottom: 22em;">
												<div class="intro-body tagline-size">
													<h1 class="intro-title mb-4">
														Effluent Treatment Plant 
													</h1>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
						<div class="carousel-item-b">
							<img
								src="img/eut-services/wwe-4.jpg"
								alt="Annual Maintenance Contract">
							<div class="intro-content display-table centered-custom">
								<div class="table-cell">
									<div class="container">
										<div class="row">
											<div
												class="col-lg-9"
												style="padding-bottom: 30em;">
												<div class="intro-body tagline-size">
													<h1 class="intro-title mb-4">
														Water Treatment Plants
													</h1>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
					<div class="row justify-content-between">
						<div class="col-md-5 col-lg-4">
							<div class="service-price d-flex justify-content-center foo">
								<div class="card-header-c d-flex">
									<div class="card-box-ico">
										<span class="fa fa-tint"></span>
									</div>
									<div class="card-title-c align-self-center">
										<h5 class="title-c">Wastewater Engineering</h5>
									</div>
								</div>
							</div>
						</div>
						<div class="col-md-7 col-lg-7 section-md-t3">
							<div class="row section-t3">
								<div class="col-sm-12">
									<div class="title-box-d">
										<h3 class="title-d">Sewage Treatment Plants</h3>
									</div>
								</div>
							</div>
							<div class="amenities-list color-text-black">
								<ul class="list-a no-margin">
									<li>Compact Sewage Treatment Plant</li>
									<li>Sewage Treatment Plant</li>
									<li>Package Sewage Treatment Plant</li>
									<li>Containerized Sewage Treatment Plant</li>
									<li>Prefabricated Sewage Treatment Plant</li>
									<li>Sewage Treatment Plant</li>
									<li>Container Sewage Treatment Plants</li>
									<li>Screw Pump</li>
									<li>Filter Press</li>
								</ul>
							</div>
							<div class="row section-t3">
								<div class="col-sm-12">
									<div class="title-box-d">
										<h3 class="title-d">Effluent Treatment Plants</h3>
									</div>
								</div>
							</div>
							<div class="amenities-list color-text-black">
								<ul class="list-a no-margin">
									<li>Zero Liquid Discharge Plant</li>
									<li>Effluent Treatment Plant</li>
									<li>Packaged Effluent Treatment Plant</li>
									<li>Packaged Effluent Treatment</li>
									<li>Prefabricated Effluent Treatment Plant</li>
								</ul>
							</div>
							<div class="row section-t3">
								<div class="col-sm-12">
									<div class="title-box-d">
										<h3 class="title-d">Waste Water Treatment Plants</h3>
									</div>
								</div>
							</div>
							<div class="amenities-list color-text-black">
								<ul class="list-a no-margin">
									<li>Effluent Treatment Plants</li>
									<li>Sewage Treatment Plants</li>
									<li>Zero Liquid Discharge Plant</li>
								</ul>
							</div>
							<div class="row section-t3">
								<div class="col-sm-12">
									<div class="title-box-d">
										<h3 class="title-d">Water Treatment Plants</h3>
									</div>
								</div>
							</div>
							<div class="amenities-list color-text-black">
								<ul class="list-a no-margin">
									<li>Membrane Filtration System</li>
									<li>Grey Water Treatment Plants</li>
									<li>Media Filtration Systems</li>
									<li>Disinfection Systems</li>
									<li>Neno Filtration</li>
									<li>MGF +ACF</li>
									<li>UV System</li>
								</ul>
							</div>
						</div>
					</div>
				</div>
				<div class="col-md-12">
					<div class="row section-t3">
						<div class="col-sm-12">
							<div class="title-box-d">
								<h3 class="title-d">Contact</h3>
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-md-6 col-lg-4">
							<h4 class="title-agent">Sales Executive</h4>
							<img
								src="img/eut-services/sales.jpg"
								alt="EutAir Sales Service"
								class="img-fluid">
							<div class="service-agent">
								<p class="color-text-black">Your enqueries are invited.
									Contact us.</p>
								<ul class="list-unstyled">
									<li class="d-flex justify-content-between"><strong>Mobile:</strong>
										<span class="color-text-black">+91 981 1860249</span></li>
									<li class="d-flex justify-content-between"><strong>Email:</strong>
										<span class="color-text-black">service@eutair.com</span></li>
								</ul>
							</div>
						</div>
						<!-- 						<div class="col-md-6 col-lg-4"></div> -->
						<div class="col-md-12 col-lg-8">
							<div class="service-contact">
								<form
									class="form-a serviceForm"
									action=""
									method="post"
									role="form">
									<div id="sendmessage">
										Thank you for contacting EutAir Equipments LLP!<br> Our
										team will shortly get back to you.
									</div>
									<div id="errormessage"></div>
									<div class="row">
										<div class="col-md-6 mb-3">
											<div class="form-group">
												<input
													type="text"
													name="name"
													class="form-control form-control-lg form-control-a"
													placeholder="Your Name"
													data-rule="minlen:4"
													data-msg="Please enter at least 4 chars">
												<div class="validation"></div>
											</div>
										</div>
										<div class="col-md-6 mb-3">
											<div class="form-group">
												<input
													type="text"
													name="company"
													class="form-control form-control-lg form-control-a"
													placeholder="Company Name"
													data-rule="required"
													data-msg="Please enter your company name">
												<div class="validation"></div>
											</div>
										</div>
										<div class="col-md-6 mb-3">
											<div class="form-group">
												<input
													type="number"
													name="mobile"
													class="form-control form-control-lg form-control-a"
													placeholder="Mobile"
													data-rule="mobile"
													data-msg="Please enter a valid phone number">
												<div class="validation"></div>
											</div>
										</div>
										<div
											class="col-md-6 mb-3"
											style="display: none;">
											<div class="form-group">
												<input
													type="text"
													name="service"
													value="WWE"
													class="form-control form-control-lg form-control-a">
											</div>
										</div>
										<div class="col-md-6 mb-3">
											<div class="form-group">
												<input
													name="email"
													type="email"
													class="form-control form-control-lg form-control-a"
													placeholder="Your Email"
													data-rule="email"
													data-msg="Please enter a valid email">
												<div class="validation"></div>
											</div>
										</div>
										<div class="col-md-12 mb-3">
											<div class="form-group">
												<textarea
													name="message"
													class="form-control"
													name="message"
													cols="45"
													rows="8"
													data-rule="required"
													data-msg="Please write something for us"
													placeholder="Message"></textarea>
												<div class="validation"></div>
											</div>
										</div>
										<div class="col-md-12">
											<button
												type="submit"
												class="btn btn-a">Send Message</button>
										</div>
									</div>
								</form>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>
	<!--/ service Single End /-->
	<!--/ footer Star /-->
	<footer> </footer>
	<!--/ Footer End /-->
	<a
		href="#"
		class="back-to-top"><i class="fa fa-chevron-up"></i></a>
	<div id="preloader"></div>
	<!-- JavaScript Libraries -->
	<script src="lib/jquery/jquery.min.js"></script>
	<script src="lib/jquery/jquery-migrate.min.js"></script>
	<script src="lib/popper/popper.min.js"></script>
	<script src="lib/bootstrap/js/bootstrap.min.js"></script>
	<script src="lib/easing/easing.min.js"></script>
	<script src="lib/owlcarousel/owl.carousel.min.js"></script>
	<script src="lib/scrollreveal/scrollreveal.min.js"></script>
	<!-- Contact Form JavaScript File -->
	<script src="js/service.js"></script>
	<script
		src="https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.6.0/slick.js"></script>
	<!-- Main Javascript File -->
	<script src="js/main.js"></script>
	<!-- Custom Javascript File -->
	<script type="text/javascript">
	fetch("./footer.html")
	  .then(response => {
	    return response.text()
	  })
	  .then(data => {
	    document.querySelector("footer").innerHTML = data;
	  });
	
	fetch("./logo.html")
	  .then(response => {
	    return response.text()
	  })
	  .then(data => {
	    document.querySelector("logo").innerHTML = data;
	  });
	
	fetch("./contact-info.html")
	  .then(response => {
	    return response.text()
	  })
	  .then(data => {
	    document.querySelector("contact-info").innerHTML = data;
	  });
	</script>
</body>
</html>
